...
class AuthJWTUseCase:
    def __init__(self, user_jwt_refresh_token_repository: IJWTRefreshTokenRepository):
    ...
    def authenticate_user(self, access_token: str) -> uuid.UUID:
        ...
        try:
            payload = jwt.decode(jwt=access_token, key=settings.jwt_secret_key, algorithms=["HS256"])
    ...
    def _generate_tokens(self, user_id: uuid.UUID) -> Tokens:
        ...
        refresh_token_entity = JWTRefreshToken(
            token_id=token_id,
            user_id=user_id,
